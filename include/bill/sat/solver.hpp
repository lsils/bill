...
#include "sat_solvers/glucose_syrup.hpp"
...
template<>
class solver<solvers::syrup_41> {
	using solver_type = Glucose::MultiSolvers;
...
#pragma region Constructors
	...
	solver(uint32_t num_threads)
	    : solver_(std::make_unique<solver_type>(num_threads))
	{}
#pragma endregion
...
#pragma region Modifiers
	...
	{
		assert(solver_->okay() == true);
		assert(assumptions.size() == 0u && "No solver support for assumptions");
		assert(conflict_limit == 0u && "No solver support for conflict limit");

		Glucose::lbool state = solver_->solve();
		if (state == Glucose::l_True) {
			result::model_type model;
			for (auto i = 0; i < solver_->model.size(); ++i) {
				if (solver_->model[i] == Glucose::l_False) {
